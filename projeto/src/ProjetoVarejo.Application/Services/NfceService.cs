...
public class NfceService
{
    ...
    private async Task<Result<NotaFiscal>> EmitirAsync(int vendaId, bool contingencia)
    {
        ...
        // Insere o QR Code (infNFeSupl) no XML assinado — obrigatório para NFC-e (modelo 65).
        try
        {
            xmlAssinado = _xmlGen.InserirQrCode(xmlAssinado, empresa, contingencia);
            nota.XmlEnviado = xmlAssinado;
        }
        catch (Exception ex)
        {
            return Result.Falha<NotaFiscal>("Erro ao inserir QR Code: " + ex.Message);
        }

...
public class KpiCard : Card
{
    ...
    private void DesenharConteudo(object? sender, PaintEventArgs e)
    {
        ...
        {
            var iconRect = new Rectangle(contentRight - 30, contentTop, 28, 28);
            using var iconPath = Tema.PathArredondado(iconRect, Tema.RaioBotao);
            g.FillPath(brush, iconPath);
            using var fontIcone = new Font("Segoe MDL2 Assets", 14);
        ...
        TextRenderer.DrawText(g, _titulo.ToUpperInvariant(), fontTitulo,
            new Rectangle(contentLeft, contentTop + 2, contentRight - contentLeft - 38, 18),
            Tema.CorTextoMedio, TextFormatFlags.Left | TextFormatFlags.EndEllipsis | TextFormatFlags.NoPrefix);

        // Largura disponível para o valor evita colidir com o ícone à direita
        var valorWidth = contentRight - contentLeft;
        using var fontValor = FonteQueCabe(g, _valor, valorWidth, 19, 11, FontStyle.Bold);
        TextRenderer.DrawText(g, _valor, fontValor,
            new Rectangle(contentLeft, contentTop + 30, valorWidth, 38),
            Tema.CorTextoEscuro, TextFormatFlags.Left | TextFormatFlags.EndEllipsis | TextFormatFlags.NoPrefix);
        ...
        if (_variacaoPct.HasValue)
        {
            ...
            var texto = $"{prefixo}{v:F1}% vs ontem";

            using var fontVar = FonteQueCabe(g, texto, valorWidth, 8.5f, 7, FontStyle.Bold);
            var y = Height - Padding.Bottom - 18;
            TextRenderer.DrawText(g, texto, fontVar,
                new Rectangle(contentLeft, y, valorWidth, 18),
                cor, TextFormatFlags.Left | TextFormatFlags.EndEllipsis | TextFormatFlags.NoPrefix);
        }
    }

    private static Font FonteQueCabe(Graphics g, string texto, int largura, float tamanhoInicial, float tamanhoMinimo, FontStyle estilo)
    {
        for (var tamanho = tamanhoInicial; tamanho >= tamanhoMinimo; tamanho -= 0.5f)
        {
            var fonte = new Font(Tema.FontFamily, tamanho, estilo);
            if (TextRenderer.MeasureText(g, texto, fonte).Width <= largura)
                return fonte;
            fonte.Dispose();
        }

        return new Font(Tema.FontFamily, tamanhoMinimo, estilo);
    }
}

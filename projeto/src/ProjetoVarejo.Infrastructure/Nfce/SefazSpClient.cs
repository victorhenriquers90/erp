...
public class SefazSpClient
{
    ...
    public async Task<RetornoSefaz> EnviarAutorizacaoAsync(
        ...
        bool usarNfce = true)
    {
        // NF-e (modelo 55) e NFC-e (65) usam o mesmo WS (NFeAutorizacao4), mas hosts diferentes.
        var host = usarNfce
            ? (homologacao ? "homologacao.nfce.fazenda.sp.gov.br" : "nfce.fazenda.sp.gov.br")
            : (homologacao ? "homologacao.nfe.fazenda.sp.gov.br" : "nfe.fazenda.sp.gov.br");
        var url = $"https://{host}/ws/NFeAutorizacao4.asmx";
